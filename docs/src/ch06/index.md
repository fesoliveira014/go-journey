...
>
> Chapter snippets are point-in-time snapshots. Chapter 6 was inserted after the first end-to-end draft, so use these tags instead of raw commit order.

At this point we have authentication, a book catalog with CRUD, and a gateway that renders it in the browser. Chapter 7 builds the Reservation service; the reservation dashboard material in this chapter is a post-Chapter 7 extension that is included here because Chapter 6 was inserted after the first end-to-end draft. Use the checkpoint tags if you want to follow the exact sequence.
...
- **Limited visibility.** An admin can manage books, but cannot see registered users. After Chapter 7, the same dashboard pattern extends to reservations across the system.
...
2. **Admin Dashboard** (Section 6.2)—A new `ListUsers` RPC on auth and gateway pages for admin visibility. The reservation dashboard portion is labeled as a post-Chapter 7 extension and adds `ListAllReservations` plus a `ReservationDetail` response that denormalizes book titles and user emails.
...
4. **Putting It Together** (Section 6.4)—A walkthrough for the full checkpoint: start the stack, create an admin, seed the catalog, register a user, make a reservation after Chapter 7, and verify everything in the admin dashboard.
