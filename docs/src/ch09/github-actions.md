...
GitHub Actions is the orchestration layer of our pipeline. It answers one question: *when* should something run, and with what environment? The build logic itself lives in the Earthfile (section 9.2). GitHub Actions triggers that logic at the right moment, supplies secrets, and runs parallel jobs across services.
