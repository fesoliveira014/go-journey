...
Earthly, which we cover in section 9.2, enforces this by running every build target inside a container. You cannot accidentally depend on your local Go installation because the build uses the Go version declared in the Earthfile, not the one in your PATH.
