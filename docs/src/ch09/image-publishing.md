# 9.5 Image Publishing & Versioning
...
The job only runs after the `ci` job completes successfully. The `ci` job runs `earthly +ci`, which executes lint and tests for all services (see Section 9.4). If any lint or test fails, `build-and-push` is skipped entirely. You never publish an image from broken code.
