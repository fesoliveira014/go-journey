...
msk_bootstrap_brokers_tls       = "b-1.library.xxxxx.kafka.us-east-1.amazonaws.com:9094,b-2.library.xxxxx.kafka.us-east-1.amazonaws.com:9094"
...
Terraform created the Secrets Manager entries for the RDS credentials in section 13.4 (those are generated by the `aws_db_instance` resource and stored automatically). The JWT signing secret, gateway flash/CSRF cookie key, internal service token, and Meilisearch master key are not generated by AWS infrastructure—you must create them manually with values you choose.
...
aws secretsmanager create-secret \
  ...
  --secret-string '{"key":"GENERATE_A_64_CHAR_HEX_KEY"}'

aws secretsmanager create-secret \
  --name library-system/internal-service-token \
  --secret-string '{"token":"GENERATE_A_STRONG_INTERNAL_TOKEN"}'
...
Replace the placeholder strings with real random values. A 128-character hex string is appropriate for a JWT secret:

```bash
openssl rand -hex 64
```

Run that command for each secret and substitute the output for the placeholder values before running the `create-secret` commands. `FLASH_COOKIE_KEY` must be hex-encoded because the gateway decodes it at startup. These values do not need to be memorized or stored anywhere outside Secrets Manager; the External Secrets Operator will sync them into the cluster automatically from this point forward.
...
| Browser session hardening | Local cookie settings | `COOKIE_SECURE=true` plus CSRF tokens on unsafe form methods |
...
The Secrets Manager entries for `library-system/jwt-secret`, `library-system/flash-cookie-key`, `library-system/internal-service-token`, and `library-system/meilisearch-key` are not managed by Terraform—you created them manually in Step 3. Delete them separately to avoid the 7-day recovery window that AWS applies to all deleted secrets:
...
aws secretsmanager delete-secret \
  --secret-id library-system/flash-cookie-key \
  --force-delete-without-recovery

aws secretsmanager delete-secret \
  --secret-id library-system/internal-service-token \
  --force-delete-without-recovery
