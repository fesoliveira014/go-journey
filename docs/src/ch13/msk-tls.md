...
|-------|---------------|---------------|-------|
| `PLAINTEXT` | Yes | No | Chapter 12 state |
| `TLS_PLAINTEXT` | Yes | Yes | Both listeners active — useful during migration |
| `TLS` | No | Yes | TLS only — Chapter 13 target state |
...
`in_cluster = true` encrypts replication traffic between the two broker nodes. This setting is independent of `client_broker` and was already enabled implicitly in Chapter 12 (it is the default). It is set explicitly here for clarity[^2].
...
MSK exposes two separate attributes for bootstrap broker strings: `bootstrap_brokers` for the plaintext listener and `bootstrap_brokers_tls` for the TLS listener. Chapter 12's `outputs.tf` already exported the plaintext string. Add the TLS equivalent:
