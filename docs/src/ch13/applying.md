# 13.6 Applying the Changes

Sections 13.2 through 13.5 introduced four independent concerns — Route 53 DNS, ACM certificate provisioning, External Secrets Operator, and MSK TLS. Each was presented in isolation so the concepts could be unpacked without the surrounding noise. Now you apply all four together.
...
Open `terraform/terraform.tfvars` and add the domain name variable introduced in section 13.2:
...
- `aws_route53_zone` — the hosted zone for your domain
- `aws_route53_record` (alias) — the A-record alias pointing your domain apex at the ALB
...
Review the plan output. If you see resources listed as destroyed unexpectedly — particularly the MSK cluster itself rather than a configuration change — stop and check that the MSK resource block in `infra/msk/main.tf` is using `aws_msk_configuration` rather than an inline `configuration_info` block that Terraform would treat as requiring replacement.
...
This applies the full overlay diff from Chapter 13 in one shot: the `SecretStore` and `ExternalSecret` resources from section 13.4, the Ingress patch with TLS annotations and the ACM certificate ARN from section 13.3, and the ConfigMap patch with the `KAFKA_BROKERS` value updated to port 9094 from section 13.5. Resources that already exist and have not changed are left untouched (Kubernetes `apply` is declarative — it reconciles, not replaces).

Expected additions in the output:

```
secretstore.external-secrets.io/aws-secrets-manager created
externalsecret.external-secrets.io/auth-secrets created
externalsecret.external-secrets.io/catalog-secrets created
externalsecret.external-secrets.io/reservation-secrets created
externalsecret.external-secrets.io/jwt-secret created
externalsecret.external-secrets.io/meilisearch-key created
ingress.networking.k8s.io/gateway configured
configmap/library-config configured
```
...
auth-secrets       aws-secrets-manager    1h                 SecretSynced   True
catalog-secrets    aws-secrets-manager    1h                 SecretSynced   True
reservation-secrets aws-secrets-manager   1h                 SecretSynced   True
jwt-secret         aws-secrets-manager    1h                 SecretSynced   True
meilisearch-key    aws-secrets-manager    1h                 SecretSynced   True
```
...
| Component | Before (Chapter 12) | After (Chapter 13) |
|-----------|---------------------|---------------------|
...
Every row in this table represents a gap that would be flagged in a security review of the Chapter 12 deployment. After this chapter, the system meets the baseline security requirements for a production deployment: encrypted public-facing traffic, managed secrets with no human-visible credential values in Kubernetes manifests or git history, and encrypted broker connections inside the cluster.

---

## Teardown

When you are done, the same `terraform destroy` command from Chapter 12 handles everything. Terraform manages the Route 53 hosted zone, the ACM certificate, the ESO Helm release and IRSA role, and the MSK configuration — all are destroyed in the correct dependency order when you run:
...
Run `kubectl delete` first to allow the ALB controller to deprovision the load balancer before Terraform destroys the VPC. The sequence is the same as Chapter 12; the resource count will be higher (roughly 57 instead of 47) to account for the Chapter 13 additions.
