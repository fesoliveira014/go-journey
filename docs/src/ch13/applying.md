...
Open `terraform/terraform.tfvars` and add the domain name variable introduced in section 13.1:
...
This applies the full overlay diff from Chapter 13 in one shot: the `SecretStore` and `ExternalSecret` resources from section 13.3, the Ingress patch with TLS annotations and the ACM certificate ARN from section 13.2, and the ConfigMap patch with the `KAFKA_BROKERS` value updated to port 9094 from section 13.4. Resources that already exist and have not changed are left untouched (Kubernetes `apply` is declarative — it reconciles, not replaces).
