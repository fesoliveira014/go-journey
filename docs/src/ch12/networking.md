...
# Note: Chapter 13 adds TLS on port 9094
...
The RDS security group opens port 5432 — PostgreSQL's default port. The MSK security group opens port 9092 — the Kafka plaintext listener. In Chapter 13, when mutual TLS is configured on the Kafka cluster, you will add a second ingress rule for port 9094 (the TLS listener). For now, plaintext within the private subnet is acceptable because no traffic crosses the public internet.
