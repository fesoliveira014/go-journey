...
1. **Add a `book.availability_changed` event type.** Right now, `UpdateAvailability` publishes a generic `book.updated` event. Create a new event type that includes only the `BookID`, `AvailableCopies`, and `TotalCopies` fields. Update the consumer (section 7.3) to handle it.
