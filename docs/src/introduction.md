...
Every chapter starts with checkpoint commands:
...
Use those tags when comparing a chapter snippet to the source tree. The default branch shows the latest final state, not the historical state for every chapter.
