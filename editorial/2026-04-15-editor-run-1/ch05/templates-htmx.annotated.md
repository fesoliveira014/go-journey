...
<!-- [STRUCTURAL] Opening grounds the reader in the Thymeleaf analog — good. The "subtle gotcha" tease in paragraph 2 motivates the Clone-per-Page section later; effective. -->
<!-- [LINE EDIT] "is a server-side template engine that produces HTML" — "that produces HTML" is mild throat-clearing; "a server-side template engine for HTML output" is tighter. Optional. -->
<!-- [COPY EDIT] "Thymeleaf" — capitalized product name. Correct. -->
<!-- [COPY EDIT] "XML-attribute-based (like Thymeleaf's `th:text`)" — compound adjective before "syntax" is correctly hyphenated (CMOS 7.81). -->
...
<!-- [LINE EDIT] Cut filler: "This section covers the template system, explains a subtle gotcha that will bite you if you do not know about it, and introduces HTMX for progressive enhancement." — "that will bite you if you do not know about it" is filler. Consider: "This section covers the template system, a subtle gotcha, and HTMX for progressive enhancement." -->
...
<!-- [STRUCTURAL] Table is the right form for the Thymeleaf↔Go mapping. Good pedagogy for the target reader. -->
A Go template is an HTML file with **actions** enclosed in double curly braces. The most important actions are:

<!-- [COPY EDIT] Table column header "Thymeleaf equivalent" — lowercase after "Thymeleaf" is fine; consistent. -->
<!-- [COPY EDIT] "`{{if .Condition}}...{{end}}`" — inline code backticks balanced; OK. -->
...
<!-- [LINE EDIT] "The dot (`.`) is the **cursor** -- it refers to the current data context." — good, clear. -->
<!-- [LINE EDIT] "This is the single most confusing thing about Go templates for newcomers." — slightly florid; "This is the most common source of confusion for newcomers." is more measured. -->
...
<!-- [LINE EDIT] "(not `text/template`)" — correct, important distinction. Keep. -->
<!-- [LINE EDIT] "you do not need to remember to call an escape function on every variable." — "remember to" is filler; "you need not call an escape function on every variable" or "no per-variable escape call needed." Stylistic. -->
<!-- [COPY EDIT] "XSS" — acronym, no expansion required given prior industry familiarity, but first-use-define is safer. Consider "cross-site scripting (XSS) attacks". -->
...
<!-- [STRUCTURAL] Order is good: concept → base.html → page-override syntax → partial example. -->
...
<!-- [STRUCTURAL] The callout on SRI + crossorigin is excellent — security depth that distinguishes this tutorial. -->
<!-- [COPY EDIT] Please verify: HTMX version 2.0.4 is a real release. HTMX 2.0 was released 2024; verify 2.0.4 specifically still resolves at https://unpkg.com/htmx.org@2.0.4 and that the integrity hash matches the published hash at htmx.org/docs/#installing. -->
<!-- [COPY EDIT] "HTMX install page" link correct. "SRI hash for every release" — confirm htmx.org publishes per-release SRI hashes. -->
<!-- [COPY EDIT] "cross-origin resources" — hyphen before "resources" (noun modifier). Correct. "cross-origin" consistent with earlier usage. -->
<!-- [LINE EDIT] "a CDN compromise can inject malicious code into every page you serve" — "into every page you serve" strong; keep. -->
<!-- [LINE EDIT] "doesn't" → prefer "does not" for formal prose consistency with rest of chapter (which uses "do not", "is not"). CMOS leaves contractions to author voice; this chapter reads as uncontracted. -->
<!-- [COPY EDIT] "In production you'd typically self-host" — same contraction note. -->
...
<!-- [LINE EDIT] "The `{{block \"title\" .}}Library System{{end}}` action defines a block named `\"title\"` with a default value of \"Library System\"." — tight. Keep. -->
...
<!-- [LINE EDIT] "Notice that the nav template accesses `.User` -- this comes from the `PageData` struct that wraps every template rendering:" — "every template rendering" is fine; "every page render" is tighter. -->
...
<!-- [LINE EDIT] "Every page gets the current user, flash message, and page-specific data." — CMOS 6.19 serial comma present. Keep. -->
<!-- [COPY EDIT] "e.g.," — CMOS 6.43 requires comma after "e.g." (and after "i.e."). Correct. -->
...
<!-- [STRUCTURAL] Crucial section; this is the signature payload of the chapter. The "DON'T DO THIS" comment in the code block is effective teaching. -->
<!-- [COPY EDIT] "Clone-per-Page" — heading-capitalization style; "Clone per Page" without hyphens is also valid. Inconsistent with "clone-per-page pattern" (lowercase, hyphenated) elsewhere. Lock one. -->
<!-- [LINE EDIT] "Here is where Go templates get tricky." — acceptable tutor-voice transition. Keep. -->
...
<!-- [LINE EDIT] "You will hit a problem: every page template defines `{{define \"content\"}}...{{end}}`. When parsed into a single set, the **last file parsed wins** -- only one \"content\" definition survives. All pages render the same content block." — tight technical explanation; good. -->
<!-- [COPY EDIT] "**last file parsed wins**" — bold-emphasized phrase; CMOS allows bold for key term emphasis. Keep. -->
...
<!-- [LINE EDIT] "In Thymeleaf, this is not an issue because each template file is an independent document that Thymeleaf processes separately." — accurate Java-bridge framing. Keep. -->
...
<!-- [COPY EDIT] "The Solution: Clone Per Page" — after-colon capitalization: CMOS 6.63 says capitalize after a colon only if what follows is a complete sentence. "Clone Per Page" is a fragment, so lowercase: "The solution: clone per page" — but this is a heading, so title-case conventions override. Keep as-is if title-case heading style. -->
<!-- [LINE EDIT] "Each clone gets its own copy of the namespace, so each page's `{{define \"content\"}}` overrides the base without affecting other pages." — good. -->
...
<!-- [STRUCTURAL] Numbered-algorithm recap after the code block is standard textbook pacing; works here. -->
The algorithm:
...
<!-- [LINE EDIT] "The key call is `tmpl.ExecuteTemplate(w, \"base.html\", pd)` -- it starts rendering from `base.html`, which pulls in the page's overridden `\"content\"` and `\"title\"` blocks." — accurate, tight. Keep. -->
...
<!-- [STRUCTURAL] The "HTMX is a small JavaScript library (14KB)" opener is punchy and strong. Good. -->
<!-- [COPY EDIT] Please verify: HTMX minified gzipped size. The number "14KB" is commonly cited; htmx.org currently states ~14KB min.gz for v2. Confirm. -->
<!-- [COPY EDIT] "14KB" — CMOS 9.16 uses numerals for measurements with unit abbreviations; space between number and unit is optional but common. Prefer "14 KB" with space. -->
<!-- [LINE EDIT] "Its philosophy is that the server should return **HTML fragments**, not JSON -- the browser swaps fragments into the DOM. This is the hypermedia approach, and it is a natural fit for a Go BFF." — strong, accurate. Keep. -->
...
<!-- [STRUCTURAL] Three-attribute table is perfect first-exposure teaching for HTMX. -->
| Attribute | Purpose | Example |
|---|---|---|
<!-- [COPY EDIT] Please verify: `hx-get`, `hx-post`, `hx-target`, `hx-swap` are the exact HTMX attribute names. Confirmed from htmx.org/docs. -->
...
<!-- [LINE EDIT] "That is the core of HTMX. There are more attributes for triggers, indicators, and history, but these three cover most use cases." — tight closer. Keep. -->
...
<!-- [STRUCTURAL] Motivated example — user story ("selects a genre") precedes the code. Good. -->
...
<!-- [LINE EDIT] "When the `<select>` changes" — "changes" assumes the default trigger. HTMX's default trigger for `<select>` is `change`. Accurate. -->
<!-- [LINE EDIT] "HTMX reads the `hx-get` attribute, appends the `<select>`'s value as a query parameter (`/books?genre=Programming`), and sends a GET request." — accurate. -->
...
<!-- [STRUCTURAL] Dual-mode handler (full page vs fragment) is the canonical HTMX server pattern. Clear. -->
<!-- [COPY EDIT] Please verify: `HX-Request` is the correct HTMX request header (HTMX sends `HX-Request: true` on AJAX requests). Confirmed per HTMX docs. -->
...
<!-- [LINE EDIT] "a bare list of `<div>` elements with no `<html>`, `<head>`, or layout" — tight. -->
<!-- [LINE EDIT] "For a full page load (or a browser with JavaScript disabled)" — aside is helpful for the progressive-enhancement argument. Keep. -->
...
<!-- [LINE EDIT] "This is progressive enhancement: the page works without JavaScript (you just get full-page reloads on filter changes), but with HTMX it feels like an SPA." — STRIP "just" per pass-2 filler rules. Consider: "This is progressive enhancement: the page works without JavaScript (you get full-page reloads on filter changes), but with HTMX it feels like an SPA." -->
<!-- [COPY EDIT] "SPA" — acronym, usually defined on first use. "single-page application (SPA)" on first appearance would be stricter CMOS. -->
...
<!-- [STRUCTURAL] Fit/not-fit pairing is good risk-honest coverage; avoids HTMX-advocacy tone. -->
...
<!-- [COPY EDIT] "HTMX is *not* a good fit when:" — italicized *not* is appropriate emphasis (CMOS 7.50). -->
HTMX is *not* a good fit when:

<!-- [COPY EDIT] "drag-and-drop" — hyphenated compound adjective before "kanban board" (CMOS 7.81). Correct. -->
<!-- [COPY EDIT] "kanban" — lowercase per Japanese-origin common noun convention (CMOS 8.58). Correct. -->
- You are building a complex, highly interactive UI (a spreadsheet, a drag-and-drop kanban board)
- You need offline support
<!-- [COPY EDIT] "React/Vue/Angular" — slashes acceptable in list context. CMOS 6.106 prefers commas but slash is idiomatic here. -->
- Your team already has deep React/Vue/Angular expertise and the application demands it

<!-- [LINE EDIT] "For our library system -- catalog browsing with filters and admin CRUD forms -- HTMX is the right tool. It adds interactivity with zero JavaScript in our codebase and keeps all rendering logic on the server." — strong summative sentence. Keep. -->
...
<!-- [COPY EDIT] Please verify all four reference URLs resolve (htmx.org/docs/, pkg.go.dev/html/template, hypermedia.systems, gowebexamples.com/templates/). -->
