...
<!-- [STRUCTURAL] Strong opener: frames the repository's single responsibility, then pivots to what the service owns. Layered-responsibility teaching is concrete and tied to specific domain invariants. Keep. -->
...
<!-- [LINE EDIT] "The service layer is where business logic lives. It sits between the transport layer (gRPC handlers) and the persistence layer (repositories), and it depends on neither." — the third clause "and it depends on neither" is technically inaccurate — the service depends on the repository interface. Consider: "...and it does not depend on gRPC or GORM — it sees both only through interfaces it owns or is handed." -->
...
<!-- [STRUCTURAL] Side-by-side comparison of Kotlin explicit-declaration vs Go structural satisfaction is one of the strongest teaching moves in the chapter. Keep. -->
...
<!-- [LINE EDIT] "Interface satisfaction is **implicit** — a type satisfies an interface simply by having the right method signatures." — banned word "simply". Rewrite: "a type satisfies an interface by having the right method signatures." -->
...
<!-- [LINE EDIT] "The GORM-backed `BookRepository` struct in `services/catalog/internal/repository/` has all these methods with matching signatures. That's enough — it satisfies the interface automatically." — clean. Leave. -->
...
<!-- [LINE EDIT] "This is a deliberate inversion of the Java pattern." — "deliberate inversion" is fine but "inversion" might confuse the reader who is trying to map this to "dependency inversion" (DIP). Consider: "This inverts the Java convention." and then explicitly connect to DIP (SOLID) for extra pedagogical payoff. -->
<!-- [STRUCTURAL] Consider a sentence connecting this explicitly to the Dependency Inversion Principle. The reader has a SOLID background; naming the principle anchors the pattern. -->
...
<!-- [LINE EDIT] "The practical consequence: you can introduce a new `BookRepository` implementation (an in-memory cache, a read-replica adapter) without touching anything in the service package. The dependency arrow points inward." — the last sentence is great ("dependency arrow points inward"). Leave. -->
...
<!-- [STRUCTURAL] Forward-reference note about EventPublisher is appropriate for a book structured around increasingly capable services. Keep. -->
...
<!-- [LINE EDIT] "This is manual dependency injection — no framework, just a constructor that takes what it needs. The service has no idea whether `repo` is backed by PostgreSQL, SQLite, or an in-memory map. That's the point." — banned "just". Rewrite: "This is manual dependency injection — no framework, a constructor that takes what it needs." -->
...
<!-- [LINE EDIT] "Most service methods are thin orchestrators. `GetBook` simply delegates:" — banned "simply". Rewrite: "`GetBook` delegates:". -->
...
<!-- [LINE EDIT] "There's no business logic here worth adding a layer for — but the layer still matters because it's the right place for logic *when it does exist*, and it decouples the gRPC handler from the repository interface. If you later need to add an audit log entry every time a book is fetched, this is where it goes, without touching the handler or the repository." — the first sentence is 50 words (over the 40 threshold). Split: "There's no business logic here worth a dedicated layer for. The layer still matters — it's the right place for logic *when it exists*, and it decouples the gRPC handler from the repository interface. If you later need to add an audit log entry every time a book is fetched, this is where it goes, without touching the handler or the repository." -->
...
<!-- [LINE EDIT] "Two things happen here that have nothing to do with SQL: the input is validated, and an invariant is enforced (`available_copies` must equal `total_copies` for a new book). The repository doesn't know about either rule — it would happily insert a book with a blank title and negative availability if asked. The service layer is the last line of defence before data reaches the database." — last sentence is effective; keep. "defence" is British; see copy edit note. -->
<!-- [COPY EDIT] "defence" — British; US "defense". Standardize across chapter. -->
...
<!-- [COPY EDIT] "available_copies" and "total_copies" in body prose: these are database column names, properly in code font. Consistent with chapter style. -->
<!-- [STRUCTURAL] Factual-consistency check: the DB migration (§2.2) enforces `available_lte_total` via CHECK constraint. The prose here says "the repository...would happily insert a book with...negative availability if asked" — the DB would actually reject it due to the `copies_non_negative` CHECK. Consider rephrasing: "the repository doesn't validate either rule; the database CHECK constraints catch the most egregious violations, but defensive validation in the service layer produces better error messages and prevents an RPC round-trip." -->
...
<!-- [LINE EDIT] "The sentinel errors live in `services/catalog/internal/model/errors.go`:" — fine. -->
...
<!-- [LINE EDIT] "The `%w` verb in `fmt.Errorf` **wraps** the sentinel error. The resulting error carries a human-readable message ('title is required') but also embeds `ErrInvalidBook` as a cause. Callers can inspect which type of error they're dealing with using `errors.Is`:" — "which type of error they're dealing with" could tighten to "its type". Suggest: "Callers can check the type using `errors.Is`:" -->
...
<!-- [COPY EDIT] Quotation-marks style: "title is required" uses curly quotes in some renderers, straight quotes in Markdown source. Straight quotes in source are correct for round-tripping to HTML/PDF — verify the build pipeline renders curly at output. -->
...
<!-- [LINE EDIT] "`errors.Is` traverses the error chain — it unwraps wrapped errors recursively until it finds a match. This means the gRPC handler doesn't need to parse the error string to decide how to respond. It just checks `errors.Is(err, model.ErrBookNotFound)` and maps to `codes.NotFound`, or checks `model.ErrInvalidBook` and maps to `codes.InvalidArgument`." — banned "just". Rewrite: "It checks `errors.Is(err, model.ErrBookNotFound)` and maps to `codes.NotFound`, or checks `model.ErrInvalidBook` and maps to `codes.InvalidArgument`." -->
...
<!-- [LINE EDIT] "Compare this to Java's exception hierarchy. The Go pattern achieves the same classification goal without inheritance or `instanceof` — just wrap a sentinel, unwrap it with `errors.Is`. [^2]" — banned "just". Rewrite: "...without inheritance or `instanceof` — wrap a sentinel, unwrap it with `errors.Is`." -->
...
<!-- [COPY EDIT] "[^2]" has leading space before the footnote marker. CMOS/Markdown convention: no space between text and footnote marker. Remove the space. -->

---

## Testing with Hand-Written Mocks

<!-- [STRUCTURAL] The "hand-written vs gomock" framing is pedagogically sound for learners. Strong justification. -->
...
<!-- [LINE EDIT] "A mock just needs to implement the same interface." — banned "just". Rewrite: "A mock needs only to implement the same interface." -->
...
<!-- [LINE EDIT] "This is not a fake of the database — it's a fake of the repository interface." — good, succinct distinction. Keep. -->
<!-- [STRUCTURAL] The phrase "fake of the repository interface" reuses "fake" in contradiction with the section heading "Hand-Written Mocks". In TDD taxonomy (Meszaros, xUnit Patterns), mocks, fakes, and stubs are distinct. This mock is actually closer to a **stateful fake** than a behavior-verifying mock. Consider either (a) acknowledging the taxonomy briefly, or (b) renaming to "hand-written fakes" for terminological accuracy. -->
...
<!-- [LINE EDIT] "Notice what's being tested: not 'does GORM insert correctly' (the repository tests cover that), but 'does the service reject invalid input'. Each test exercises one business rule. When a test fails, you know exactly which rule broke." — good. Leave. -->
...
<!-- [LINE EDIT] "Why hand-written over `mockgen`? Small, focused interfaces (often 1–3 methods) are Go's norm [^1] — they're trivial to implement by hand. The mock above also stores actual state in a map, which makes tests more realistic than `gomock`'s call-expectation style. And there's no generated code to decode when something fails." — good. Leave. -->
<!-- [COPY EDIT] "[^1]" has leading space before the footnote marker (line 193). Remove leading space. -->
...
<!-- [STRUCTURAL] This exercise is a highlight — it drives the reader through the full TDD loop (add error, write failing test, implement). Keep the structure verbatim. -->
...
<!-- [LINE EDIT] "This has a bug: a book can be deleted even if copies are currently checked out (i.e., `available_copies < total_copies`). That means outstanding reservations would reference a book that no longer exists." — "bug" is a charged word for what is arguably a missing feature. Consider: "This is incomplete: ..." or "This leaves a gap in the invariants: ..." -->
...
<!-- [COPY EDIT] "Run `go test ./...` from `services/catalog/` — the test should fail..." — convention: `go test ./...` backticked. Good. -->
...
<!-- [STRUCTURAL] The solution introduces a subtle TOCTOU (time-of-check to time-of-use) issue: between GetByID and Delete, a reservation could be created. Worth a one-line forward reference: "In Chapter 7, when inter-service events arrive, this check becomes a race. We'll revisit with transactional guardrails." -->
...
<!-- [LINE EDIT] "The mock's `UpdateAvailability` modifies `AvailableCopies` in the in-memory map, so the test correctly reflects the post-checkout state. No database required." — good. -->
...
<!-- [LINE EDIT] "The service layer is complete. It validates input, enforces invariants, wraps domain errors, and coordinates repository calls — all without coupling itself to GORM or gRPC. The next section connects everything: the gRPC handler calls the service, which calls the repository, and `main.go` wires all three together with dependency injection." — good. Leave. -->
...
<!-- [COPY EDIT] Please verify footnote URLs live and canonical; the "Error handling and Go" blog was partially superseded by the Go 1.13 errors package blog post — consider linking to the newer one or adding it as a supplementary footnote. -->
