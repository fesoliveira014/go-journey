...
<!-- [STRUCTURAL] Excellent discussion-style chapter close. Doesn't pretend to teach implementation; explains the model, contrasts with the push pipeline from 13.8, lists pros/cons, gives adoption signals. The "foundation is already in place" close is a nice payoff. Keep as-is. -->
...
<!-- [LINE EDIT] "perfectly reasonable" — "perfectly" is filler. Suggest "is reasonable". -->
...
<!-- [COPY EDIT] "ArgoCD" or "Argo CD"? The official project styles itself "Argo CD" (with space). Many sources use "ArgoCD" colloquially. Pick canonical form. -->
...
<!-- [COPY EDIT] Please verify: GitOps coined by Weaveworks in 2017 (Alexis Richardson blog post). Confirmed. -->
<!-- [COPY EDIT] "four principles" — the OpenGitOps project (CNCF) formally documents four principles: declarative, versioned/immutable, pulled automatically, continuously reconciled. The text below maps to these but uses different paraphrases. Worth a footnote tying them. -->
...
<!-- [COPY EDIT] Please verify: Argo CD was developed at Intuit and contributed to CNCF. Graduated CNCF status (Dec 2022). Confirmed. -->
...
spec:
  ...
  syncPolicy:
    automated:
      ...
      selfHeal: true
```
<!-- [COPY EDIT] Please verify: `argoproj.io/v1alpha1` is current as of 2026; confirm whether v1beta1 has shipped. Argo CD has remained on v1alpha1 historically. -->
<!-- [COPY EDIT] `path: k8s/overlays/production` here vs. chapter elsewhere using `deploy/k8s/overlays/production`. Unify. -->
...
<!-- [LINE EDIT] "are how you enforce" — passive-ish; consider "AppProjects enforce that team A cannot accidentally modify team B's namespace". -->
...
<!-- [LINE EDIT] "a single button click" → "a single click". -->
...
<!-- [COPY EDIT] Please verify: Flux CNCF graduated status. Graduated Nov 2022. Confirmed. -->
<!-- [LINE EDIT] "is largely organizational preference" — slight hedge; OK. -->
...
<!-- [COPY EDIT] Please verify: ArgoCD default polling interval is 3 minutes (180s `timeout.reconciliation`). Confirmed. -->
<!-- [COPY EDIT] "image-tags.yaml" — non-canonical filename; `kustomization.yaml` is where `images:` lives. The text earlier said "kustomize edit set image" updates `kustomization.yaml`. Reconcile. -->
...
<!-- [COPY EDIT] "the EKS OIDC configuration from section 13.8 are removed" — subject-verb agreement. "configuration … are" → "configuration … is" or "EKS OIDC configurations … are". Pick one. -->
...
- name: Update image tag in overlay
  run: |
    ...
    git push
```
<!-- [COPY EDIT] `library/catalog` (the path) and `catalog` (the source key) — earlier files use `library-system/catalog`. Unify path. -->
<!-- [COPY EDIT] `cd k8s/overlays/production` — chapter elsewhere uses `deploy/k8s/overlays/production`. Inconsistent. -->
<!-- [COPY EDIT] Conventional Commits "chore:" — fine. Keep. -->
...
<!-- [COPY EDIT] "infinite loop risks" — "infinite loop" is two words; OK. Could also use "feedback loop". -->
<!-- [LINE EDIT] "the pipeline should be configured to skip runs triggered by the bot user's commits" — passive. → "configure the pipeline to skip runs triggered by the bot user's commits". -->
...
<!-- [COPY EDIT] Please verify: "Argo CD Image Updater" capitalization — official project name. Also note its sub-CNCF status. -->
...
<!-- [LINE EDIT] "The most operationally significant difference." — sentence fragment; intentional emphasis. Acceptable. -->
...
<!-- [COPY EDIT] No `staging` overlay exists in the chapter. Forward-references a config that doesn't yet exist. Soften: "...a hypothetical `staging` overlay in addition to production..." -->
...
<!-- [COPY EDIT] "is not trivial" — double negative; consider "is non-trivial" or "is substantial". -->
...
<!-- [COPY EDIT] "ArgoCD cannot deploy itself" — actually it can manage its own manifests after bootstrap (the "App of Apps" pattern). Slight imprecision. Reword: "ArgoCD cannot install itself the first time." -->
...
<!-- [LINE EDIT] "actually exist" — "actually" is filler. → "in response to problems that exist". -->
<!-- [LINE EDIT] "Complexity should be introduced in response to problems that actually exist, not in anticipation of problems that might exist." — passive. → "Introduce complexity in response to problems that exist, not in anticipation of problems that might." -->
...
<!-- [LINE EDIT] "post-mortems" — CMOS 7.85 prefers "postmortems" (closed compound). Some style guides allow hyphen. Consistent. -->
...
<!-- [LINE EDIT] "you should not feel compelled to add it" — soft and kind. Keep. -->
...
<!-- [COPY EDIT] "k8s/overlays/staging/" — staging overlay was never built in this chapter. Either qualify "if you add a staging overlay" or remove. -->
...
<!-- [COPY EDIT] `k8s/overlays/production` here vs `deploy/k8s/overlays/production` elsewhere. Pick one. -->
...
<!-- [LINE EDIT] Strong closing paragraph. Keep. -->
