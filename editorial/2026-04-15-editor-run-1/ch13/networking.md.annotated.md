...
<!-- [STRUCTURAL] The topology-before-code structure is strong: diagram → reasoning → module usage → security groups → what-will-be-created summary. Readers get the "why" before the "how". Keep. -->
<!-- [COPY EDIT] Section heading uses " — " (em dash) while sibling sections use ":" or plain space. index.md lists "13.2 VPC and Networking". Unify. CMOS 6.85: em dash with no surrounding spaces in titles is unusual. -->
...
<!-- [LINE EDIT] "need subnets with routing rules that prevent them from being directly reachable from the internet" — slightly long. → "need subnets whose routing prevents direct reachability from the internet." -->
...
<!-- [COPY EDIT] "Availability Zones" — AWS capitalizes; you use lowercase later ("availability zone"). CMOS 8.66: capitalize when proper-noun-like product feature. Prefer consistent form; recommend "Availability Zone" throughout, matching AWS docs. -->
<!-- [COPY EDIT] Please verify: "an Availability Zone is a physically separate data center" — AWS documentation says "one or more discrete data centers with redundant power, networking, and connectivity." An AZ can comprise multiple DCs. Soften to "one or more physically separate data centers". -->
...
<!-- [COPY EDIT] "Internet Gateway" — AWS capitalizes as product name. "internet" lowercase (CMOS 8.190). Apply consistently. -->
<!-- [LINE EDIT] "it is itself a managed AWS resource that needs a route" — wordy. → "itself a managed AWS resource that needs a route". -->
...
<!-- [LINE EDIT] "A few details worth noting." — fragment (see index.md). Add "are". -->
...
<!-- [COPY EDIT] Please verify: "~$65/month each" for NAT Gateway — 730 hrs × $0.045/hr ≈ $32.85. $65 appears incorrect by ~2x. index.md's NAT row shows $33. Reconcile. -->
<!-- [LINE EDIT] "In production, revisit this." — good terse close. Keep. -->
...
<!-- [COPY EDIT] "65,536" — CMOS 9.56: comma in 4+ digit numerals. OK. -->
...
<!-- [LINE EDIT] "Rather than assembling a VPC from individual … clean interface." — 50 words. Works as one breath but could split. Acceptable. -->
...
```
<!-- [COPY EDIT] Please verify: `public_subnet_tags` and `private_subnet_tags` are valid inputs on terraform-aws-modules/vpc/aws v5. Yes, present in module's variables.tf. OK. -->
<!-- [COPY EDIT] `var.environment` used in tags but not declared in this file; terraform-fundamentals.md doesn't show it. Remind reader where variables come from. -->

Walk through each block.
<!-- [LINE EDIT] "Walk through each block." — imperative fragment. Prefer: "Each block deserves a walkthrough." OR "Let's walk through each block." Acceptable as is in tutorial tone. -->
...
<!-- [COPY EDIT] "`var.aws_region`" but the HCL block above uses `var.region` (terraform-fundamentals.md) or unspecified. Inconsistent variable naming. -->
...
<!-- [LINE EDIT] "constrains it to one, shared NAT Gateway" — comma is unnecessary. → "constrains it to one shared NAT Gateway". -->
...
<!-- [COPY EDIT] Please verify: AWS docs state this tag is only required for self-managed cluster discovery; EKS managed clusters since 2020 do not strictly require it. However, AWS LB Controller does still benefit from it. Footnote worth adding. -->
...
<!-- [STRUCTURAL] A minor redundancy risk: msk.md repeats the MSK security group definition. Decide once: keep SG with VPC file (here) OR with the resource file (msk.md). Don't do both. Currently both files define `aws_security_group.msk` — that's a compile error. Cross-reference note essential. -->
...
resource "aws_security_group" "msk" {
  ...
  description = "Allow Kafka inbound from EKS nodes"
  vpc_id      = module.vpc.vpc_id
...
```
<!-- [COPY EDIT] DUPLICATE DEFINITION: `aws_security_group.msk` is also declared in msk.md. One must be removed. This is a bug. -->
<!-- [COPY EDIT] msk.md references `aws_security_group.eks_nodes.id` for its MSK SG source; this file uses `module.eks.node_security_group_id`. Inconsistent. Pick one. -->
<!-- [COPY EDIT] DEPENDENCY: both SGs reference `module.eks.node_security_group_id` but this file is `vpc.tf` — usually created before EKS. That is a classic circular-dependency issue. Either: (a) use CIDR range; (b) declare the EKS node SG separately and reference it from both; (c) use `aws_security_group_rule` to separate SG and its rules. Worth an explicit discussion note. -->
...
The RDS security group opens port 5432 — PostgreSQL's default port. The MSK security group opens port 9092 — the Kafka plaintext listener. In Chapter 14, when mutual TLS is configured on the Kafka cluster, you will add a second ingress rule for port 9094 (the TLS listener). For now, plaintext within the private subnet is acceptable because no traffic crosses the public internet.
<!-- [COPY EDIT] "mutual TLS" — MSK's `TLS` client-broker mode is server-auth TLS, not mutual (mTLS) by default. mTLS is a separate feature (TLS with client certs). Verify Chapter 14's scope and adjust wording. -->
...
- 2 security groups
<!-- [COPY EDIT] "4 route tables" — default is typically 3 (1 public + 2 private, with single NAT) or 4 (1 public + 2 private + 1 default). Please verify count produced by terraform-aws-modules/vpc/aws. -->
<!-- [COPY EDIT] Numerals: CMOS 9.3 spells out zero–ninety-nine in nontechnical prose; these are technical counts and numerals are fine. Keep. -->
...
terraform plan
```
<!-- [COPY EDIT] `infra/terraform` directory here; `terraform/` in terraform-fundamentals.md; `infrastructure/` in cicd.md. Three different paths. Unify. -->
...
<!-- [FINAL] Footnotes uncited inline. Same pattern across chapter. -->
