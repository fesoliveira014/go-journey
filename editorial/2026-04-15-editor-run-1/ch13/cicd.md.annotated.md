...
<!-- [STRUCTURAL] Excellent section. Flow: why-OIDC → how OIDC works (sequence diagram) → Terraform → workflow YAML → optional PR plan → rollback. The sequence diagram earning its keep — it clarifies the exchange. One weakness: "Optional: Terraform plan on pull requests" is shown as pseudo-code comment block. Could be moved to an appendix or removed if truly out-of-scope. -->
<!-- [COPY EDIT] Heading "13.8 CI/CD Pipeline" vs index.md "13.8 — Continuous Deployment with GitHub Actions". Unify. -->
...
<!-- [LINE EDIT] "none of it is doing anything useful until a deployment pipeline closes the loop" — 16 words, crisp. Keep. -->
<!-- [COPY EDIT] "an RDS database per service" — rds.md provisions 3 instances (auth, catalog, reservation); search and gateway have none. The roster matches. OK. -->
...
<!-- [COPY EDIT] "A failed rollout rolls back automatically" — the workflow below uses `kubectl rollout status --timeout=300s`, which fails the job but does NOT trigger rollback. kubectl does not auto-rollback on failed rollouts; that requires either `kubectl rollout undo` on failure, a deployment strategy config, or Helm/ArgoCD. Overclaim. Fix wording. -->
...
<!-- [COPY EDIT] Please verify: GitHub Actions OIDC provider launched Oct 2021 (GA). Correct. -->
...
<!-- [COPY EDIT] "You get branch-scoped permissions with no extra work" — conversational; acceptable tutor tone. Keep. -->
...
<!-- [COPY EDIT] Please verify: "15 minutes by default" — aws-actions/configure-aws-credentials default session duration is 1 hour (3600s), not 15 minutes. Short-lived but not that short. The STS minimum is 15 minutes. Confirm and correct. -->
<!-- [LINE EDIT] "If the token is leaked, it is useless before an attacker can act on it." — slight overstatement (15 minutes is still actionable). Soften: "...it has a narrow exploitation window." -->
...
```
<!-- [COPY EDIT] Please verify: as of 2023, AWS no longer strictly requires the `thumbprint_list` for well-known providers like GitHub Actions (they validate against a curated list). Many AWS docs now show thumbprint_list as optional/bypassed. Worth a footnote. -->
<!-- [COPY EDIT] Please verify: `data "tls_certificate"` with URL fetches the cert chain; using `[0].sha1_fingerprint` gets the leaf cert. Historically GitHub's OIDC endpoint sits behind a CDN; the correct thumbprint is the root, and fetching `[0]` may give the leaf. Confirm current HashiCorp TLS provider behavior. -->
...
```
<!-- [COPY EDIT] Please verify: `aws_eks_access_entry` and `aws_eks_access_policy_association` resource arguments. Confirmed correct. -->
<!-- [COPY EDIT] Please verify: `AmazonEKSEditPolicy` ARN format. Correct: `arn:aws:eks::aws:cluster-access-policy/AmazonEKSEditPolicy`. -->
<!-- [COPY EDIT] Please verify: ECR actions list — `ecr:GetAuthorizationToken`, `ecr:BatchCheckLayerAvailability`, `ecr:InitiateLayerUpload`, `ecr:UploadLayerPart`, `ecr:CompleteLayerUpload`, `ecr:PutImage`, `ecr:BatchGetImage`, `ecr:GetDownloadUrlForLayer`. All valid. -->
...
<!-- [COPY EDIT] Please verify: EKS access entries API released ~Dec 2023 with kube-apiserver 1.28+. Confirm. -->
...
```
<!-- [COPY EDIT] Directory `infrastructure`; other files use `terraform/` or `infra/terraform`. Unify. -->
<!-- [COPY EDIT] Using `-target` is discouraged by HashiCorp for normal use (it breaks the dependency graph). Acknowledge this is a limited bootstrap command. -->
...
<!-- [LINE EDIT] "This avoids double-building, ensures ECR contains exactly the same layer digests as GHCR, and keeps the ECR push atomic with the deployment." Good sentence. Keep. -->
...
```
<!-- [COPY EDIT] "library/${SERVICE}" — source image name for `kustomize edit set image` must match the base manifest image name. production-overlay.md uses `library-system/${SERVICE}`. Inconsistent. -->
<!-- [COPY EDIT] "--name library-cluster" — deploying.md uses `library-production`; eks.md uses `local.cluster_name`. Unify. -->
<!-- [COPY EDIT] Please verify: `imranismail/setup-kustomize@v2` — popular community action; alternative is `aquaproj/aqua-installer` or official kustomize release. Confirm v2 is current. -->
<!-- [COPY EDIT] Please verify: `aws-actions/configure-aws-credentials@v4` default session duration. Default 1h. Earlier text says 15 minutes. Reconcile. -->
<!-- [COPY EDIT] Please verify: ECR login token TTL 12 hours. Confirmed. -->
<!-- [COPY EDIT] `ECR_REGISTRY/library/${SERVICE}` path here; ecr.md uses `library-system/${SERVICE}` path. Unify registry path. -->
...
<!-- [COPY EDIT] "is optional but recommended" — aws-actions/configure-aws-credentials@v4 generates a default session name. The reasoning is correct. OK. -->
...
<!-- [STRUCTURAL] This whole commented-out workflow is probably worth dropping or moving to an appendix. Commented YAML in running prose is hard to read; readers can find examples in the GitHub Actions ecosystem. -->
...
#             });
```
<!-- [COPY EDIT] "const plan = fs.readFileSync('terraform/plan.txt', 'utf8');" — working-directory is `infrastructure`, not `terraform/`. Path mismatch. -->
...
<!-- [COPY EDIT] Please verify: `revisionHistoryLimit` default is 10. Confirmed per Kubernetes docs. -->
...
<!-- [LINE EDIT] "deployed by a pipeline, with no credentials stored anywhere that do not expire" — negation-heavy. → "deployed by a pipeline that stores no long-lived credentials". -->
