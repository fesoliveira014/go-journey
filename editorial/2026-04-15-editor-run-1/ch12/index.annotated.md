...
<!-- [STRUCTURAL] Opening does its job: it frames the "why" by contrasting Compose with Kubernetes, and lists the specific Compose limitations Kubernetes addresses. Good motivation. -->
<!-- [LINE EDIT] "exactly right" → "the right tool" (reduce idiom stacking). -->
<!-- [COPY EDIT] Consider en dash between clauses joined with a long range/pause, or a semicolon. CMOS 6.85/6.78. -->
...
<!-- [LINE EDIT] "It is a platform for running containerized workloads across a cluster of machines, with built-in self-healing…" → "It runs containerized workloads across a cluster of machines with built-in self-healing…" (active, tighter). -->
<!-- [COPY EDIT] "rolling updates, service discovery, autoscaling, and declarative configuration management" — serial comma present, good (CMOS 6.19). -->
...
<!-- [STRUCTURAL] This paragraph is the chapter's "promise" sentence. Keep it — it's effective. -->
...
<!-- [STRUCTURAL] Good anchor: leaning on what the reader already knows. -->
...
<!-- [COPY EDIT] Table: use consistent backticks. "container" is wrapped in backticks but "Pod" is not — both are conceptual here, but the other rows use plain terms on the Kubernetes side. Consider dropping the backticks from "container" for consistency, OR backticking "Pod", "Deployment", etc. Stay consistent within the column. -->
...
<!-- [LINE EDIT] "controls startup order — service B won't start until service A's container exists. Kubernetes does not have startup ordering." → "controls startup order: service B won't start until service A's container exists. Kubernetes has no startup ordering." (tighter; avoids the "does not have" construction). -->
<!-- [COPY EDIT] "readiness probes" in bold, but later references in prose use lowercase. Decide: bolded first mention is fine; confirm subsequent uses match. -->
<!-- [COPY EDIT] "an HTTP endpoint, a TCP connection, or an exec command" — serial comma present. -->
<!-- [COPY EDIT] "the pod" — use Pod (capital P) per the established convention when referring to the Kubernetes API resource. Be consistent with the Kustomize/Deployment term conventions. CMOS: trademark/product conventions. -->
...
<!-- [LINE EDIT] "binds a container port to a host port, making the service reachable at `localhost:8080`" → tighter: "maps a container port onto a host port so the service is reachable at `localhost:8080`". -->
...
<!-- [LINE EDIT] "PersistentVolumeClaims are notably heavier than Compose volumes." — "notably" is filler. Drop. -->
<!-- [COPY EDIT] "the Container Storage Interface (CSI)" — standard convention; good. Keep the expansion the first time only. -->
...
<!-- [STRUCTURAL] Good progression — from the Compose-bridge table to the control plane components. -->
<!-- [COPY EDIT] "section 12.1" — lowercase s is fine here (CMOS 8.178 considers cross-references lowercase unless followed by a number that is part of a proper noun). Be consistent across the chapter. -->
...
<!-- [COPY EDIT] "API Server" — Kubernetes docs style it "API server" (server lowercase). Worth aligning with the upstream docs style. Please verify: https://kubernetes.io/docs/concepts/overview/components/#kube-apiserver uses "kube-apiserver" / "API server". -->
<!-- [LINE EDIT] "Every interaction — from `kubectl` on your laptop to a controller inside the cluster — goes through the API server." → good sentence, keep. -->
...
<!-- [COPY EDIT] "etcd" — correctly lowercase. CNCF project name. Good. -->
...
<!-- [LINE EDIT] "considering resource requests, node selectors, affinity rules, taints and tolerations" → add serial comma: "considering resource requests, node selectors, affinity rules, and taints and tolerations" (last pair is a unit, so the CMOS fix is either to keep it as a pair or rewrite; consider: "considering resource requests, node selectors, affinity rules, and taint/toleration constraints"). -->
...
<!-- [LINE EDIT] "runs a collection of control loops, each watching a specific resource type and reconciling the cluster's actual state toward the desired state." — strong; keep. -->
...
<!-- [LINE EDIT] "The diagram shows that everything flows through the API server." — the diagram is above; the diagram "shows" is redundant. → "Everything flows through the API server." -->
...
<!-- [LINE EDIT] "watches the API server for pods assigned to its node, instructs the container runtime (typically containerd) to pull images and start containers, monitors their health, and reports status back to the API server" — clear. -->
<!-- [COPY EDIT] "kubelet" — lowercase is correct; it is the Kubernetes component name. -->
...
<!-- [COPY EDIT] "iptables or IPVS" — "iptables" lowercase is fine (tool name). "IPVS" uppercase is correct. -->
<!-- [LINE EDIT] "transparent to your application — your code connects to the Service name, and the network layer handles the rest." — good. -->
...
<!-- [STRUCTURAL] This is arguably the most important conceptual section in the chapter. It earns its position here. Good. -->
The most important conceptual shift from Compose to Kubernetes is the move from **imperative** to **declarative** operations.

<!-- [LINE EDIT] "If a container crashes later, nothing brings it back unless you run the command again." — correct, succinct. -->
...
<!-- [LINE EDIT] "you write a manifest that says "I want three replicas of the catalog service running, using image `library/catalog:v1.2.0`, with 256 MB of memory and 100m of CPU."" — long, but effective. Keep. -->
<!-- [COPY EDIT] "256 MB" vs. the YAML "256Mi" used elsewhere. MB is decimal (1,000,000), Mi is binary (1,048,576). Both are acceptable in prose but note the mismatch with manifests if a reader cross-references. Consider "256 MiB" for consistency with the manifests. -->
<!-- [COPY EDIT] "100m of CPU" — millicores, lowercase m is correct Kubernetes convention. A brief parenthetical (= 0.1 CPU) would help first-time readers. -->
...
<!-- [LINE EDIT] "This reconciliation loop runs forever. It is not a one-time action but an ongoing process." → one sentence: "This reconciliation loop runs forever — an ongoing process, not a one-time action." -->
...
<!-- [LINE EDIT] "For a system built to run continuously and recover from failure, this model is fundamentally more reliable than issuing imperative commands." → "For a system built to run continuously and recover from failure, this model is far more reliable than issuing imperative commands." (cut "fundamentally" — weaker than "far"). -->
...
<!-- [STRUCTURAL] Reference-style list section. OK here because the chapter needs a shared vocabulary going forward. Could be tightened — every entry is ~100 words. Consider trimming. -->
...
<!-- [LINE EDIT] "In practice, most pods contain a single application container; the multi-container pattern (called a sidecar) is used for cross-cutting concerns like log collection or service mesh proxies." → "(called the sidecar pattern)" — "sidecar" is the pattern name, not a label for the multi-container setup itself. CMOS 7.81: sidecar not hyphenated when nominalized. -->
...
<!-- [LINE EDIT] "It manages a ReplicaSet (a group of identical pods) and handles rolling updates:" — good. -->
...
<!-- [LINE EDIT] "Pods are ephemeral: they are created and destroyed as replicas scale up and down, and each gets a new IP address when it starts." → tighten: "Pods are ephemeral: created and destroyed as replicas scale, each assigned a fresh IP at start." -->
<!-- [COPY EDIT] "only reachable" — "is only reachable inside the cluster" — CMOS 5.186 dangling "only": "is reachable only inside the cluster." Better. -->
...
<!-- [LINE EDIT] "like a Deployment but designed for stateful workloads — databases, message brokers, anything that needs stable network identities, ordered startup and shutdown, and persistent storage" — clear. -->
<!-- [COPY EDIT] "postgres-0, postgres-1" — inline code would aid scanning: `postgres-0`, `postgres-1`. -->
...
<!-- [LINE EDIT] "Secrets are base64-encoded (not encrypted by default at rest, though encryption can be enabled)" — the parenthetical is awkward; readers will misread it as "encryption can be enabled for base64." Reword: "Secrets are base64-encoded and, by default, stored unencrypted at rest (etcd encryption can be enabled separately)." -->
<!-- [COPY EDIT] "base64-encoded" — hyphenated compound modifier before noun; CMOS 7.81 ok. When used predicatively ("the value is base64 encoded"), no hyphen. -->
...
<!-- [LINE EDIT] "requests to `library.example.com/api` go to the gateway service; requests to `library.example.com/auth` go to the auth service" — fine; or tighten: "`…/api` → gateway; `…/auth` → auth." Narrative form is more welcoming. -->
...
<!-- [COPY EDIT] "nginx" — when referring to the product, NGINX (all-caps) is the upstream style. "nginx" is the binary/package name. Be consistent throughout chapter. Current chapter mixes both. Please verify: https://nginx.org uses "NGINX" for the product. -->
<!-- [COPY EDIT] "ReadWriteOnce", "ReadWriteMany" — API enum values, keep as-is. -->
...
<!-- [LINE EDIT] "This is the primary way to deploy changes." — drop "This is" opening; "The primary way to deploy changes." Also acceptable to keep. -->
...
<!-- [COPY EDIT] Code fences lack a language tag. Use ```bash for shell examples (consistency with kind-setup.md and deploying.md). Applies to all code blocks in this section. -->
```
...
<!-- [COPY EDIT] "catalog-7d9f4b-xvz2k" — note this is a pod name with deployment hash; consistent with kubectl naming. OK. -->
...
<!-- [LINE EDIT] "One concept that runs through all of these commands: **namespaces**." — strong; keep. -->
<!-- [COPY EDIT] "RBAC (role-based access control)" — parenthetical expansion is good on first mention; CMOS 10.3. -->
...
<!-- [COPY EDIT] List items use em dash; consistent. Good. -->
...
<!-- [LINE EDIT] "This separation is not just organizational." → "This separation is more than organizational." (less negative phrasing). -->
<!-- [COPY EDIT] "RBAC (role-based access control)" — already expanded above? No — first expansion is here. Good location. -->
...
<!-- [LINE EDIT] "as a service in `data` is reachable at `postgres.data.svc.cluster.local` from any namespace" — good. -->
...
<!-- [STRUCTURAL] Roadmap section uses bolded section numbers; matches a textbook convention and aids navigation. Keep. -->
<!-- [LINE EDIT] "the fastest path to a real cluster without a cloud account" — good closer. -->
...
<!-- [COPY EDIT] "locally-built images" — compound adjective before noun, hyphenated. CMOS 7.81 ok. -->
<!-- [LINE EDIT] "makes them Kubernetes-ready: health check endpoints (for liveness and readiness probes), graceful shutdown handling, and configuration via environment variables rather than hardcoded values" — clear. -->
...
<!-- [COPY EDIT] "PostgreSQL, Kafka, and Meilisearch" — serial comma; good. -->
...
<!-- [COPY EDIT] "Kubernetes' built-in configuration management tool" — Kubernetes' (possessive with apostrophe after s per CMOS 7.19 for singular ending in s) ok. -->
<!-- [LINE EDIT] "create a base configuration and two overlays — development and production" — earlier section refers to the local overlay as "local" (not "development"). Reconcile terminology. The overlay is named `local` throughout the rest of the chapter. Change here to "local and production." -->
...
<!-- [LINE EDIT] "ties everything together" — mild cliché; consider "brings the pieces together" — but minor. -->
...
<!-- [LINE EDIT] "every service you have built will be running in a real Kubernetes cluster — self-healing, namespace-isolated, and configured exactly as they would be in a production cloud deployment." — "they" refers to "every service" (singular); subject-verb agreement issue. → "services you have built will be running… — self-healing, namespace-isolated, and configured exactly as they would be…" -->
<!-- [COPY EDIT] "it is the production target that all your work has been building toward" — comma splice after "not a replacement for that workflow, it is". Consider: "…not a replacement for that workflow; it is the production target that all your work has been building toward." CMOS 6.57. -->
...
<!-- [FINAL] All three footnote URLs point to active Kubernetes docs. No obvious broken links at writing time. -->
