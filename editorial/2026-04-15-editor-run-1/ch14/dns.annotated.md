...
<!-- [STRUCTURAL] Opening grounds the reason for Route 53 concretely — the ALB hostname and ACM certificate requirement. Good motivation before the refresher. -->
<!-- [LINE EDIT] "That URL works, but it is not something you would print on a business card, give to a user, or embed in a mobile app." → memorable; keep. -->
<!-- [COPY EDIT] "TLS" — first use in this section; already defined in index.md. Consider whether each section should redefine abbreviations for readers landing via direct link. CMOS 10.3 permits both approaches; decide chapter-wide. -->
<!-- [COPY EDIT] "ACM" — first use in dns.md. Expand once: "AWS Certificate Manager (ACM)". -->
<!-- [LINE EDIT] "a certificate for `*.us-east-1.elb.amazonaws.com` is not yours to request" — crisp. Keep. -->
...
<!-- [STRUCTURAL] Second paragraph gives three benefits of a custom domain; fine. Consider bulleting if expanded further. -->
<!-- [LINE EDIT] "When your ALB is replaced — during a cluster migration, a region failover, or a blue/green deployment — the DNS record is the only thing that changes." → keep; precise. -->
...
<!-- [LINE EDIT] "The Terraform is concise: one data source to look up your hosted zone, one data source to look up the ALB, and one record that points your domain at the load balancer using an AWS alias record." → "The Terraform is concise: one data source for the hosted zone, one for the ALB, and one record pointing your domain at the load balancer via an AWS alias." (38 → 28 words). -->
...
<!-- [STRUCTURAL] The DNS refresher is well-scoped (A, CNAME, alias) and bounded. Good audience fit for an engineer who knows HTTP but not DNS internals. Keep. -->
<!-- [COPY EDIT] "`8.8.8.8`" — code-styled IP; fine. -->
<!-- [COPY EDIT] "Route 53, in your case." — fragment; idiomatic. Keep. -->
...
<!-- [COPY EDIT] "IPv4" — correct capitalization. OK. -->
<!-- [LINE EDIT] "Hardcoding an IP in an A record and hoping it never changes is not a strategy you want to rely on." → keep for voice. -->
...
<!-- [COPY EDIT] "e.g., `example.com` without a subdomain" — "e.g.," with comma per CMOS 6.43. Good. -->
<!-- [COPY EDIT] "SOA and NS record" — should be "SOA and NS records" (plural) — two record types, at least one of each. Minor. -->
<!-- [LINE EDIT] "the presence of a CNAME at the apex would be technically invalid" → "a CNAME at the apex would be invalid" (drops "technically" and "the presence of"). -->
...
<!-- [LINE EDIT] "Route 53 resolves it internally and returns the current A record of the target" → "Route 53 resolves it internally and returns the target's current A records" (plural; ALBs have multiple A records). -->
<!-- [COPY EDIT] "Alias records are free (Route 53 does not charge per query for alias records pointing to AWS resources)" — accurate and well-qualified. Good. -->
...
<!-- [COPY EDIT] "internet" lowercase — CMOS 8.190. Good. -->
<!-- [LINE EDIT] "serves DNS responses to the open internet — any resolver worldwide can query it" — "open" is redundant with "any resolver worldwide". Drop "open". -->
...
<!-- [COPY EDIT] "externally-registered" — CMOS 7.89: hyphenated adverb-adjective compound with "-ly" adverb is NOT hyphenated. Change to "externally registered". -->
...
<!-- [LINE EDIT] "returns all values for a name without any health-check or geographic logic" → "returns all values for a name without health-check or geographic logic" (drop "any"). -->
...
<!-- [STRUCTURAL] Code block followed by block-by-block walkthrough. Good pedagogy. Keep. -->
...
<!-- [COPY EDIT] "You will set it in `terraform.tfvars` alongside `cluster_name` and `github_repo`." — assumes those vars exist from earlier chapters. Fine. -->
...
<!-- [LINE EDIT] "If you have not yet created the hosted zone — for example, you just registered a fresh domain — you would comment out this data source..." — "for example, you just registered a fresh domain" could be "(e.g., just registered a fresh domain)" for brevity. Keep as is for voice. -->
<!-- [COPY EDIT] "NS records it assigns" — NS on first use here; earlier index.md noted expansion. OK now. -->
...
<!-- [COPY EDIT] "even though this is an alias" — "though" might be idiomatic; "even though" is wordy. Keep per tutor voice. -->
...
<!-- [COPY EDIT] "probe the ALB's health endpoints" — technically Route 53 alias health evaluation relies on the ALB's own target-group health, not a separate probe. QUERY: verify phrasing — "evaluate the ALB's target-group health" may be more precise. -->
<!-- [COPY EDIT] Please verify: with `evaluate_target_health = true` and an ALB alias, Route 53 evaluates the ALB's target group health (ALB itself is considered healthy if at least one target is healthy). Phrasing "probe the ALB's health endpoints" is imprecise. -->
...
<!-- [COPY EDIT] "Application Load Balancer" — define acronym ALB on first use (already used earlier in chapter, acceptable). -->
...
<!-- [STRUCTURAL] This is a strong section — calls out a real trap and explains it honestly. Keep prominent. -->
<!-- [LINE EDIT] "There is a sequencing dependency here that Terraform's dependency graph cannot automatically resolve." → "Terraform's dependency graph cannot resolve this sequencing automatically." (15 → 9 words). -->
...
<!-- [COPY EDIT] "30–60 seconds" — en dash for ranges per CMOS 6.78. Good. -->
<!-- [LINE EDIT] "everything proceeds in a single apply" — keep. -->
...
<!-- [COPY EDIT] "`null_resource`" — backticks. OK. -->
<!-- [LINE EDIT] "Terraform has a `null_resource` that can depend on external commands via a `local-exec` provisioner." → keep. -->
<!-- [LINE EDIT] "makes the apply non-idempotent" — precise; good. -->
...
<!-- [LINE EDIT] "useful in CI scripts that need to pass the URL downstream for smoke tests or notifications" — precise and useful. Keep. -->
...
<!-- [COPY EDIT] Code block lacks language hint (```bash). Most code blocks in this file use no language or "bash" inconsistently. Consider adding `bash` for shell blocks for syntax highlighting consistency. -->
```
cd terraform
terraform plan
terraform apply
...
<!-- [COPY EDIT] "under ten seconds" — CMOS 9.2: spell out zero–one hundred in prose. "under ten seconds" is correct. Consistent with later "30–60 seconds" (numerals with units/ranges — CMOS 9.7). Good. -->
...
<!-- [STRUCTURAL] Verification block with dig/nslookup/curl is textbook-correct and matches reader expectations. Keep. -->
<!-- [LINE EDIT] "DNS propagation from Route 53 is usually near-instantaneous..." — 45 words, borderline. Consider a split after "300 seconds." -->
<!-- [COPY EDIT] "near-instantaneous" — hyphenated compound adjective before implied noun; OK per CMOS 7.81 — but "nearly instantaneous" reads better. Optional. -->
<!-- [COPY EDIT] "negative TTL of the zone's SOA record" — accurate. The SOA record's minimum field controls negative caching TTL. Good. -->
...
<!-- [COPY EDIT] The answer TTL in the example is "60" but prose says default TTL is 300. Alias records to an ALB return the target's TTL (typically 60s), so this is accurate — worth a one-line clarifying note: "Alias records return the target's TTL (typically 60 seconds for ALBs), not the 300-second default." -->
...
<!-- [STRUCTURAL] "If you don't own a domain" section lands well for the learning-project audience. Keep. -->
<!-- [LINE EDIT] "services like **FreeDNS** (freedns.afraid.org) offer free subdomains under community-owned domains." → keep. -->
<!-- [COPY EDIT] "FreeDNS" — product capitalization correct. URL in plain text rather than link — consistent if done across chapter; otherwise Markdown-link the URL. -->
...
<!-- [LINE EDIT] "The catch is that FreeDNS is not Route 53, so you would use their web interface to create the CNAME record rather than the Terraform code in this section." → keep; tells the reader exactly what they give up. -->
...
<!-- [COPY EDIT] "$12/year" — CMOS 9.21: currency symbol + numeral is fine. "per year" is an alternative. Consistent with "$0.50" in index.md. Good. -->
<!-- [COPY EDIT] Please verify: `.com` domains registered via Route 53 currently $14/year (per AWS pricing 2025). "$12/year" may be outdated. -->
<!-- [LINE EDIT] "The domain pays for itself in the time you save debugging certificate validation issues." → keep; nice voice. -->
